...
class Admin_dashboard extends CI_Controller {
	...
	/*public function lock_any_store()
	{
		date_default_timezone_set('America/Mexico_City');
        ...
        $orders = $this->Orders->order_today_by_store($store_id,$any_date);
        ...
        $this->session->set_userdata(array('message'=>'Tienda Bloqueada !'));
        redirect('Admin_dashboard');
	}*/

    public function lock_store($store_id)
    {
        date_default_timezone_set('America/Mexico_City');
        ...
        $this->Stores->lock_store($store_id);


        $orders = $this->Orders->order_today_by_store($store_id);
        $this->load->helper('file');
        if($orders)
        {
            foreach ($orders as $order)
            {
                ...
                $file_name = 'VTA'.$orderObj['order']->store_id.'_'.date('Y').date('m').date('d').'.txt';
                $folio_timbrado = '';
                $date_timbrado ='';
                if($orderObj['order']->timbrado=='0')
                {
    ...
    public function unlock_store($store_id)
    {
        if (!$this->auth->is_logged() )
        {
            $this->output->set_header("Location: ".base_url().'admin', TRUE, 302);
        ...
        $this->Stores->unlock_store($store_id);

        $this->session->unset_userdata('message');
        $this->session->set_userdata(array('message'=>'Tienda Desbloqueada !'));
        redirect('Admin_dashboard');
    ...
    public function index(){
        ...
        {
            if($this->session->userdata('store_id')=='' || $this->session->userdata('store_id')==null)
            {
                $this->logout();
