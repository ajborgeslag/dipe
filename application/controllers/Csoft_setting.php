...
class Csoft_setting extends CI_Controller {
    ...
    {
        if ($_FILES['conf_cert_path']['name']) {
            $config['upload_path'] = 'application/libraries/facturacion/certificados/';
            $config['allowed_types']        = '*';

            $this->load->library('upload', $config);
            if ( ! $this->upload->do_upload('conf_cert_path'))
            {
                //$error = array('error' => $this->upload->display_errors());
                $this->session->set_userdata(array('error_message'=> $this->upload->display_errors()));
                redirect(base_url('Csoft_setting/invoice_setting'));
            }
            else
            {
                $conf_cert_path_file =$this->upload->data();
                $conf_cert_path = $conf_cert_path_file['file_name'];
            }
        }

        if ($_FILES['conf_key_path']['name']) {
            $config['upload_path'] = 'application/libraries/facturacion/certificados/';
            $config['allowed_types']        = '*';

            $this->load->library('upload', $config);
            if ( ! $this->upload->do_upload('conf_key_path'))
            {
                //$error = array('error' => $this->upload->display_errors());
                $this->session->set_userdata(array('error_message'=> $this->upload->display_errors()));
                redirect(base_url('Csoft_setting/invoice_setting'));
            ...
            {
                $conf_key_path_file =$this->upload->data();
                $conf_key_path = $conf_key_path_file['file_name'];
            }
        }
        ...
        $data=array(
            'pac_usuario' => $this->input->post('pac_usuario'),
            'pac_pass' => $this->aescrypto->encriptar($this->input->post('pac_pass'), $key),
            'conf_pass' => $this->aescrypto->encriptar($this->input->post('conf_pass'), $key),
            'conf_rfc' => $this->aescrypto->encriptar($this->input->post('conf_rfc'), $key),
            'conf_cert_path' => (!empty($conf_cert_path)?$conf_cert_path:$this->input->post('old_conf_cert_path')),
            'conf_key_path' => (!empty($conf_key_path)?$conf_key_path:$this->input->post('old_conf_key_path')),
        );

        $this->Soft_settings->update_invoice_setting($data);
    ...
    public function invoice_setting(){
        $content = $this->lsoft_setting->invoice_setting_form();
        $this->template->full_admin_html_view($content);
    }
