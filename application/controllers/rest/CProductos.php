...
class CProductos extends REST_Controller
{
    function __construct() {
		...
		$this->load->model('website/customer/rest/Product');
    }
	
	public function get_search_products_post()
    {
        $store_id = $this->input->post('store_id');
        $product 		= $this->input->post('product');

        $products = $this->Product->search_products($store_id,$product);
    ...
    public function get_products_by_category_get($store_id,$category_id)
    {
        ...
        $category = $this->Categories->category_by_id($category_id);
    ...
    public function get_products_get($store_id)
    {
        $config["total_rows"] 	= $this->Categories->product_count($store_id)/*($this->Categories->product_count($store_id)<=100) ? $this->Categories->product_count($store_id):100*/;
        $config["per_page"] 	= 20;

        $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;

        $products = $this->Categories->product($config["per_page"],$page,$store_id);
    ...
    {
        $product = $this->Products_model->product_info_by_store($product_id,$store_id);
