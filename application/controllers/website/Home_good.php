		...
		if ($product_id) {
            ...
            if (!empty($tax_info)) {
                if (($product_details->onsale_store == 1)) {
                    $cgst = ($tax_info->tax_percentage * $product_details->onsale_price_store)/100;
                    $cgst_id = $tax_info->tax_id;
                }else{
                    $cgst = ($tax_info->tax_percentage * $product_details->price_store)/100;
                    $cgst_id = $tax_info->tax_id;
                }
            }
            ...
            if (!empty($tax_info)) {
            	if (($product_details->onsale == 1)) {
			 		$igst = ($tax_info->tax_percentage * $product_details->onsale_price_store)/100;
			 		$igst_id = $tax_info->tax_id;
			 	}else{
			 		$igst = ($tax_info->tax_percentage * $product_details->price_store)/100;
			 		$igst_id = $tax_info->tax_id;
			 	}
		 	}
    ...
    public function integrarVentas()
    {
        ...
        if($orders)
        {
            foreach ($orders as $order)
            {
                ...
                $subTotal = round($orderObj['order']->total_amount - $orderObj['order']->tax_amount,2);
                $impuestos = round($orderObj['order']->tax_amount,2);
                $total = $orderObj['order']->total_amount;
                $cadenaHeader = 'HDR'.'|'.$orderObj['order']->store_id.'|'.date('Y').date('m').date('d').'|'.$orderObj['order']->order.'|'.$timbrado.'||||'.$subTotal.'|'.$impuestos.'|'.$total;
                ...
                foreach ($order_details as $od)
                {
                    ...
                    {
                        $amount = $od['amount'];
                        $totalWithOutTax = round(($od['total_price']-($od['quantity']*$od['discount']))-$od['amount'],2);
                    }
                    else
                    {
                        $amount = 0;
                        $totalWithOutTax = round(($od['total_price']-($od['quantity']*$od['discount'])),2);
                    }
                    //$promo = $this->Catalogues->product_by_store($orderObj['order']->store_id,$od['product_id']);

                    $total = round(($od['total_price']-($od['quantity']*$od['discount'])),2);
                    $precioUnitario = $od['rate']-$od['discount'];
