...
class Login extends CI_Controller {
	...
	public function do_login()
	{
		...
		if ( $error != '' ){
		    ...
		    if($this->session->has_userdata('last_url'))
            {
            ...
            $customer = $this->Logins->user_by_email($email);

            if($items==0 && $customer!=false)
            {
                //echo "aki";
                $this->session->unset_userdata('store_id');
                $this->session->set_userdata('store_id',$customer['store_id']);
            }

			$this->session->set_userdata(array('message'=>display('login_successfully')));
			$this->output->set_header("Location: ".$last_url/*base_url()*/, TRUE, 302);
        }
	}
