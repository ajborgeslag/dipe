...
$this->Settings->send_mail($this->input->post('email'),display('activate_user'),$this->config->item('base_url').'activate_user/'.$customer_id);
	...
	public function user_signup()
	{
		...
		if ($result) {
			...
			$this->Settings->send_mail($this->input->post('email'),display('activate_user'),'Gracias por registrarse en su tienda Dipepsa, para terminar su registro es necesario que haga click en el siguiente link <a href="'.$this->config->item('base_url').'activate_user/'.$customer_id.'">'.$this->config->item('base_url').'activate_user/'.$customer_id.'</a>');
			redirect(base_url('user_registered'));
		}else{
			$this->session->set_userdata(array('error_message'=>display('you_have_not_sign_up')));
			redirect(base_url('signup'));
