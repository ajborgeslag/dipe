...
class Auth {
	...
	{
		$CI =& get_instance();
        if ($CI->session->userdata('user_type')==1 || $CI->session->userdata('user_type')==2 || $CI->session->userdata('user_type')==4)
		{
			return true;
		}
		return false;
    ...
    public function is_admin_cont()
    {
        $CI =& get_instance();
        if ($CI->session->userdata('user_type')==1 || $CI->session->userdata('user_type')==2 || $CI->session->userdata('user_type')==4 || $CI->session->userdata('user_type')==6)
        {
            return true;
        }
        return false;
	...
	function check_admin_cont_auth($url='')
    {
        if($url==''){$url = base_url().'admin';}
        $CI =& get_instance();
        if ((!$this->is_logged()) || (!$this->is_admin_cont()))
        {
            $this->logout();
            $error = display('you_are_not_authorised');
            $CI->session->set_userdata(array('error_message'=>$error));
