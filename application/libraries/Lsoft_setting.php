...
class Lsoft_setting {
    ...
    public function invoice_setting_form()
    {
        $CI =& get_instance();
        $CI->load->model('Soft_settings');
        $setting_invoice = $CI->Soft_settings->retrieve_setting_invoice();

        $data = array(
            'title' => 'Datos de facturación',
            'id' => $setting_invoice[0]['id'],
            'pac_usuario' => $setting_invoice[0]['pac_usuario'],
            'pac_pass' => $setting_invoice[0]['pac_pass'],
            'conf_cert_path' => $setting_invoice[0]['conf_cert_path'],
            'conf_key_path' => $setting_invoice[0]['conf_key_path'],
            'conf_pass' => $setting_invoice[0]['conf_pass'],
            'conf_rfc' => $setting_invoice[0]['conf_rfc']
        );

        $setting = $CI->parser->parse('soft_setting/invoice_setting',$data,true);
        return $setting;
    }
