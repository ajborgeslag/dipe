...
class Facturacion {
    ...
    public function facturar($datos_fac,$order,$receptor,$comprobante)
    {
        ...
        $datos['xml_debug']=FCPATH.'assets/timbrados/'.$order['order']->order_id.'_test.xml';
        ...
        $datos['PAC']['usuario'] = $datos_fac[0]['pac_usuario'];
        $datos['PAC']['pass'] = $datos_fac[0]['pac_pass'];
        ...
        $datos['conf']['cer'] = FCPATH.'application/libraries/facturacion/certificados/'.$datos_fac[0]['conf_cert_path'];
        $datos['conf']['key'] = FCPATH.'application/libraries/facturacion/certificados/'.$datos_fac[0]['conf_key_path'];
        $datos['conf']['pass'] = $datos_fac[0]['conf_pass'];
		...
		$datos['factura']['subtotal'] = round($order['order']->total_amount - $order['order']->tax_amount,2);
		//$datos['factura']['subtotal'] = 0.01 * (int)(($order['order']->total_amount - $order['order']->tax_amount)*100);
        ...
        $datos['emisor']['rfc'] = $datos_fac[0]['conf_rfc'];
		...
		foreach($order_details as $o)
        {
            ...
            $datos['conceptos'][$i]['valorunitario'] = round($o['rate']-$o['discount'],2);
			/*die($o['rate']-$o['discount']);*/
            ...
            if($o['amount'])
            {
				...
				//$datos['conceptos'][$i]['importe'] = $o['total_price']-($o['quantity']*$o['discount'])-$o['amount'];
				...
				//$datos['conceptos'][$i]['Impuestos']['Traslados'][0]['Importe'] = $o['amount'];
			...
			else
			{
                ...
                $datos['conceptos'][$i]['Impuestos']['Traslados'][0]['Base'] = round(($o['total_price']-($o['quantity']*$o['discount'])),2);
                $datos['conceptos'][$i]['Impuestos']['Traslados'][0]['Impuesto'] = '002';
                $datos['conceptos'][$i]['Impuestos']['Traslados'][0]['TipoFactor'] = 'Tasa';
                $datos['conceptos'][$i]['Impuestos']['Traslados'][0]['TasaOCuota'] = '0.000000';
                $datos['conceptos'][$i]['Impuestos']['Traslados'][0]['Importe'] = 0;
			}
		...
		if($order['order']->tax_amount)
		{
			...
			//$datos['impuestos']['TotalImpuestosTrasladados'] = $order['order']->tax_amount;	
		}
		else
        {
            $datos['impuestos']['translados'][0]['impuesto'] = '002';
            $datos['impuestos']['translados'][0]['tasa'] = '0.000000';
            $datos['impuestos']['translados'][0]['importe'] = 0;
            $datos['impuestos']['translados'][0]['TipoFactor'] = 'Tasa';

            $datos['impuestos']['TotalImpuestosTrasladados'] = 0;
            //$datos['impuestos']['TotalImpuestosTrasladados'] = $order['order']->tax_amount;
        }
...
//print_r($datos);
	//	echo "------"."</br></br>";
		...
		if(array_key_exists('cfdi', $res) && $res['cfdi']!='')
        {
            ...
            //return true;
			return $this->datos_order_factura($datosHTML['rutaxml']);
        }
        else
        {
            return false;
	...
	public function datos_order_factura($xml_archivo)
    {
        date_default_timezone_set('America/Mexico_City');
        ...
        foreach ($xml->xpath('//cfdi:Comprobante') as $cfdiComprobante)
        {
            ...
            $date_timbrado = date("Y-m-d H:i:s");

            foreach ($xml->xpath('//tfd:TimbreFiscalDigital') as $tfd)
            {
                $folio_timbrado= $uuid=$tfd['UUID'];
            }
        }

        $datos = array(
            'forma_pago' => $forma_pago,
            'date_timbrado' => $date_timbrado,
            'folio_timbrado' => $folio_timbrado,
            'timbrado' =>1
        );
...
$html .= '<tr>';
$html .= '<td>';
$html .= '<strong><span style="color: #ff0000;font-size:8pt">FACTURA</span></strong>';
...
$html .= '<table style="width: 100%; font-size:7pt;margin-top: 30px" border="1" cellspacing="0">';
$html .= '<tbody>';
$html .= '<tr>';
...
$html .= '<td style="width: 10%;"><img src="'.$urlQr.'" /></td>';
...
$html .= '<p style="text-align: center;font-size:7pt;margin-top:10px;">&nbsp;</p>';
$html .= '<h4 style="font-size:7pt;margin-top:10px;">SELLO DIGITAL DEL EMISOR</h4>';
$html .= '<p style="font-size:7pt;margin-top:10px;">'.$timbre_selloCFD.'</p>';
//$html .= '<p>&nbsp;</p>';
$html .= '<h4 style="font-size:7pt;margin-top:10px;">SELLO DIGITAL DEL SAT</h4>';
$html .= '<p style="font-size:7pt;margin-top:10px;">'.$timbre_selloSAT.'</p>';
$html .= '<h4 style="font-size:7pt;margin-top:10px;">CADENA ORIGINAL DEL COMPLEMENTO DE CERTIFICACION DIGITAL DEL SAT</h4>';
$html .= '<p style="font-size:7pt;margin-top:10px;">'.$cadena_sat.'</p>';
$html .= '<p style="font-size:7pt;margin-top:10px;;text-align: center;">ESTE DOCUMENTO ES UNA REPRESENTACION IMPRESA DE UN CFDI</p>';
$html.='</div>';
		return $html;
