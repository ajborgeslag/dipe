...
class Orders extends CI_Model {
    ...
    public function order_today_by_store($store_id)
    {
	...
	public function order_date_by_store($date,$store_id)
    {
		$this->db->select('o.order_id');
        $this->db->from('order o');
        $this->db->where('o.date_only',$date);
        $this->db->where('o.pagado',1);
        $this->db->where('o.store_id',$store_id);
        $this->db->order_by('o.order','asc');
        $result = $this->db->get();
    ...
    public function order_customer_id($order_id)
    {
        $this->db->select('customer_information.*,order_state.*,o.order');
        $this->db->from('order o');
        $this->db->join('customer_information','o.customer_id = customer_information.customer_id');
        $this->db->join('order_state','o.order_state_id = order_state.id');
    ...
    public function order_payment_method_by_id($order_id)
    {
        $this->db->select('a.forma_pago');
        $this->db->from('order a');
    ...
    {
        $this->db->select('a.*,b.tax_amount,s.store_address,s.zip,s.store_name');
        $this->db->from('order a');
        $this->db->join('order_tax_col_summary b','b.order_id = a.order_id','left');
		$this->db->join('store_set s','s.store_id = a.store_id','left');
        $this->db->where('a.order_id',$idOrder);
        ...
        if ($order!=null) {
			...
			$this->db->join('unit u','u.unit_id = b.unit','left');
            ...
            for($i=0;$i<count($orderDetails);$i++)
            {
                ...
                $this->db->from('order_tax_col_details a');

                $this->db->where('a.order_id',$orderDetails[$i]['order_id']);
                $this->db->where('a.product_id',$orderDetails[$i]['product_id']);

                $tax = $this->db->get()->result_array();

                if($tax)
                {
                    $orderDetails[$i]['amount'] = $tax[0]['amount'];
                }
                else
                {
                    $orderDetails[$i]['amount'] = null;
                }
            }

            $datos = array();
            $datos['order'] = $order;
            $datos['order_details'] = $orderDetails;
    ...
    public function update_timbre($order_id,$data)
    {
        $this->db->where('order_id',$order_id);
        $this->db->update('order',$data);
