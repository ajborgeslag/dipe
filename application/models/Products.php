...
class Products extends CI_Model {
	...
	public function product_report()
    {
        $query=$this->db->select('
					product_information.clave_interna,
					product_information.product_name,
					tax_product_service.tax_percentage,
				')

            ->from('product_information')
            ->join('tax_product_service', 'tax_product_service.product_id = product_information.product_id','left')
            ->get();
        if ($query->num_rows() > 0) {
	...
	public function product_list_withOut_paginate($store_id)
    {
        $query=$this->db->select('
					catalogue_products.price,
					catalogue_products.stock,
					product_information.clave_interna,
            ...
            ->join('catalogue', 'catalogue.catalogue_id = catalogue_products.catalogue_id','left')
            ->join('store_set', 'store_set.catalogue_id = catalogue.catalogue_id','left')
    ...
    public function update_precio_stock($catalogue_product_id,$data)
    {
        $this->db->where('catalogue_product_id',$catalogue_product_id);
        $this->db->update('catalogue_products',$data);
    }
    ...
    //Count Product
    public function product_image_entry($data)
    {
        $this->db->select('*');
	...
	//Count Product
	public function product_entry($data,$data_impuesto)
	{
        $this->db->select('*');
		...
		if ($query->num_rows() > 0) {
            ...
            /* agregar el impuesto del producto */
            $this->db->insert('tax_product_service',$data_impuesto);
            /** */
	...
	{
		$this->db->select('product_information.*,tax_product_service.t_p_s_id,tax_product_service.tax_id,tax_product_service.tax_percentage');
		$this->db->from('product_information');
        $this->db->join('tax_product_service', 'product_information.product_id = tax_product_service.product_id','left');
	...
	//Update Categories
	public function update_product($data,$product_id,$data_impuesto,$t_p_s_id)
	{
        ...
        if($t_p_s_id!='')
        {
            ...
            $this->db->update('tax_product_service',$data_impuesto);
        }
        else
        {
            $this->db->insert('tax_product_service',$data_impuesto);
	...
	public function delete_product($product_id)
	{
		...
		if($affected_row == 0) {
            ...
            /*Eliminar el impuesto de este producto*/
            $this->db->where('product_id',$product_id);
