...
<body>
    <header class="section-header">
        ...
        <div class="modal fade" id="show_info" tabindex="-1" role="dialog" aria-labelledby="Notificación" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div id="show_info_body" class="modal-body">
                    </div>
        ...
        <div class="modal fade" id="show_info_reload" tabindex="-1" role="dialog" aria-labelledby="Notificación" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header btn-danger">
                        <h5 class="modal-title">Notificación</h5>
                    ...
                    <div id="show_info_body_reload" class="modal-body">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal" aria-label="Cerrar">Cerrar</button>
                    </div>
    ...
    $('#show_info_reload').on('hidden.bs.modal',function(){
    ...
    function cart_btn(product_id){
        ...
        if (product_id == 0) {
            ...
            $('#show_info_body').html('<?php echo display('ooops_something_went_wrong')?>');
            $('#show_info').modal('show');
            return false;
        }
        if (qnty <= 0) {
            //alert('<?php echo display('please_keep_quantity_up_to_zero')?>');
            $('#show_info_body').html('<?php echo 'La cantidad de producto tiene que ser mayor que 0.' ?>');
            $('#show_info').modal('show');
            return false;
        }
        ...
        if (variant != 'undefine') {
            if (variant <= 0) {
                ...
                $('#show_info_body').html('<?php echo display('please_select_product_size')?>');
                $('#show_info').modal('show');
                return false;
            }
        }
        ...
        if(parseInt(qnty)>stock){
            ...
            $('#show_info_body').html('<?php echo display('there_are_not_the_Qnty_in_stock')?>');
            $('#show_info').modal('show');
            return false;
        }
        ...
        $.ajax({
            ...
            error: function() {
                ...
                $('#show_info_body').html('Request Failed, Please check your code and try again!');
                $('#show_info').modal('show');
            }
        });
    }
    ...
    $('body').on('click', '.review', function() {
        ...
        if (review_msg == 0) {
            ...
            $('#show_info_body').html('Por favor, escriba un comentario !');
            $('#show_info').modal('show');
            return false;
        }
        ...
        if (customer_id == 0) {
            ...
            $('#show_info_body').html('Por favor, para evaluar el producto debe estar logueado');
            $('#show_info').modal('show');
            return false;
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == '1') {
                    ...
                    $('#show_info_body_reload').html('Gracias por emitir su comentario !');
                    $('#show_info_reload').modal('show');
                    ///window.load();
                }else if(data == '2'){
                    $('#review_msg').val('');
                    //alert('Gracias, ya ha emitido un comentario antes !');
                    //window.load();
                    $('#show_info_body_reload').html('Gracias, ya ha emitido un comentario antes !');
                    $('#show_info_reload').modal('show');
                }else if(data == '3'){
                    $('#review_msg').val('');
                    //alert('<?php echo display('please_login_first')?>');
                    //window.load();
                    $('#show_info_body_reload').html('<?php echo display('please_login_first')?>');
                    $('#show_info_reload').modal('show');
                }
            },
            error: function() {
